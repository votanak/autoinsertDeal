...
function copyTextToInput() {
  let tileItem, inputField;
  console.log(document.title);
  // Попробуем найти название сделки
  if (document.title === 'Новая задача') {
    tileItem = document.querySelectorAll('span[data-role="tile-item-name"]')[0];
    console.log(tileItem);
    inputField = document.querySelector('input[data-bx-id="task-edit-title"]');
    console.log(inputField);
  } else {
    tileItem = document.querySelectorAll('span[data-role="tile-item-name"]')[4];
    inputField = document.querySelector('input.main-mail-form-field-value');
  }
  ...
  if (tileItem && inputField) {
    ...
    console.log(
      'Не удалось найти нужные элементы на странице или во фрейме ',
      document.title,
    );
  }
}
